...
+ (BOOL)createTable
{
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (NSString *column in resultArray) {
            ...
            NSString *sql = [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ ",[NSStringFromClass(self.class) lowercaseString],fieldSql];
            if (![db executeUpdate:sql]) {
                res = NO;
                *rollback = YES;
                return ;
...
//        NSString *proType = [[dict objectForKey:@"type"] objectAtIndex:index];
//        NSString *fieldSql = [NSString stringWithFormat:@"%@ %@",column,proType];
//        NSString *sql = [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ ",[NSStringFromClass(model.class) lowercaseString],fieldSql];
//        if (![db executeUpdate:sql]) {
//            return NO;
//        }
//    }
...
- (BOOL)saveOrUpdate
{
    ...
    //NSLog(@"%ld",[primaryValue integerValue]);
...
- (BOOL)save
{
    NSString *tableName = [[NSStringFromClass(self.class) lowercaseString] lowercaseString];
...
+ (NSArray *)findByCriteria:(NSString *)criteria WithProperty:(NSString *)property
{
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    NSMutableArray *users = [NSMutableArray array];
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = [NSStringFromClass(self.class) lowercaseString];
        NSString *sql = [NSString stringWithFormat:@"SELECT %@ FROM %@ %@",property,tableName,criteria];
        FMResultSet *resultSet = [db executeQuery:sql];
        while ([resultSet next]) {
            JKDBModel *model = [[self.class alloc] init];
                NSString *columeType = [model.columeTypes objectAtIndex:[model.columeNames indexOfObject:property]];
                if ([columeType isEqualToString:SQLTEXT]) {
                      [users addObject:[resultSet stringForColumn:property]];
                } else {
                    [users addObject:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:property]]];
                }
            FMDBRelease(model);
        }
    }];
...
+ (NSArray *)findDictByCriteria:(NSString *)criteria WithProperty:(NSArray *)propertys
{
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    NSMutableArray *users = [NSMutableArray array];
    NSMutableString *properties = [NSMutableString string];
    for (NSString *property in propertys) {
        if ([propertys indexOfObject:property] != (propertys.count - 1)) {
            [properties appendFormat:@"%@ ,",property];
        }else{
            [properties appendFormat:@"%@ ",property];
        }
    }
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = [NSStringFromClass(self.class) lowercaseString];
        NSString *sql = [NSString stringWithFormat:@"SELECT %@ FROM %@ %@",properties,tableName,criteria];
        FMResultSet *resultSet = [db executeQuery:sql];
        while ([resultSet next]) {
            JKDBModel *model = [[self.class alloc] init];
            NSMutableDictionary *dict = [NSMutableDictionary dictionary];
            for (int i=0; i< propertys.count; i++) {
                NSString *columeName = [propertys objectAtIndex:i];
                NSString *columeType = [model.columeTypes objectAtIndex:[model.columeNames indexOfObject:columeName]];
                if ([columeType isEqualToString:SQLTEXT]) {
                    [dict setObject:[resultSet stringForColumn:columeName] forKey:columeName];
                } else {
                    [dict setObject:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:columeName]] forKey:columeName];
                }
            }
