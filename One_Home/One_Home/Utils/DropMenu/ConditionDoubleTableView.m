...
//    self = [super initWithFrame:CGRectMake(0, STATUS_AND_NAVIGATION_HEIGHT + 40, frame.size.width, frame.size.height - STATUS_AND_NAVIGATION_HEIGHT - 40)];
    self = [super init];
    if (self) {
        self.view.frame = CGRectMake(0, FILTER_HEIGHT, frame.size.width, frame.size.height - STATUS_AND_NAVIGATION_HEIGHT - FILTER_HEIGHT);
        ...
        UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, frame.size.height / 2, frame.size.width, frame.size.height / 2 - STATUS_AND_NAVIGATION_HEIGHT - FILTER_HEIGHT)];
    ...
    if (tableView == _firstTableView) {
        ...
        switch (indexPath.section) {
            case 0:
            {
                ...
                cell.textLabel.text = [dic valueForKey:@"cname"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == _firstTableView && _leftArray.count > 0) {
        ...
        [self returnSelectedValue:indexPath.row andText:[tableView cellForRowAtIndexPath:indexPath].textLabel.text];
    }
...
- (void)showSingleOrDouble {
    if (_leftArray.count <= 0) {
        ...
        _secondTableView.frame = CGRectMake( 0, 0, OneScreenW, OneScreenH / 2);
    } else {
        [_firstTableView setHidden:NO];
        _secondTableView.frame = CGRectMake(OneScreenW / 2, 0, OneScreenW, OneScreenH / 2);
    }
}
...
//返回选中位置
- (void)returnSelectedValue:(NSInteger)index  andText:(NSString *)text{
