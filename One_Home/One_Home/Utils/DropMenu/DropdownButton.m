...
- (id)initDropdownButtonWithTitles:(NSArray*)titles{
    self = [super initWithFrame:CGRectMake(0, 0, OneScreenW, BUTTON_HEIGHT)];
    if (self) {
        _isButtion = NO;
        _count = titles.count;
...
- (void)addButtonToView:(NSArray *)titles {
    ...
    UIView *bottomLine = [[UIView alloc] initWithFrame:CGRectMake(0, BUTTON_HEIGHT, OneScreenW, 1)];
...
- (UIButton *)makeButton:(NSString *) title andIndex:(int)index{
    ...
    float padding = (width - (_image.size.width + [UILabel labelAutoCalculateRectWith:title font:OneFont(13.0f) maxSize:CGSizeMake(width, CGFLOAT_MAX)].width)) / 2;
    ...
    [button setImageEdgeInsets:UIEdgeInsetsMake(11, [UILabel labelAutoCalculateRectWith:title font:OneFont(13.0f) maxSize:CGSizeMake(width, CGFLOAT_MAX)].width + padding + 5, 11, 0)];
