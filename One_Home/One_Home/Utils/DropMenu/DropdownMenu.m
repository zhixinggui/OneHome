...
- (id)initDropdownWithButtonTitles:(NSArray*)titles andLeftListArray:(NSArray*)leftArray andRightListArray:(NSArray *)rightArray {
    self = [super init];
    if (self) {
        self.view.frame = CGRectMake(0, 0, OneScreenW, MENU_HEIGHT + 25);
        _button = [[DropdownButton alloc] initDropdownButtonWithTitles:titles];
        _button.delegate = self;
        _tableView = [[ConditionDoubleTableView alloc] initWithFrame:CGRectMake(0, 0, OneScreenW, OneScreenH) andLeftItems:leftArray andRightItems:rightArray];
...
- (void)showMenu:(NSInteger)index {
    ...
    if (self.delegate  && [self.delegate respondsToSelector:@selector(didSelectedBtn:)]) {
        [self.delegate didSelectedBtn:_buttonSelectedIndex];
    }
}
...
- (void)reduceBackgroundSize {
    [self.view setFrame:CGRectMake(0, 0, OneScreenW, 35)];
}
